...
class Client:
    ...
    #获取用户的互粉好友详细信息列表
    def get_friend_bilateral(self,uid):
        try:
            return self.client.friendships.friends.bilateral.get(uid=uid,count=200)
        except Exception ,e:
			print e
			current_index = int(random.random()*100 % self.length)
			self.client=self.access_client(current_index)
			return self.get_friend_bilateral(uid)
    
    #获取用户的互粉好友ID列表
    def get_friend_bilateral_id(self,uid):
        try:
            return self.client.friendships.friends.bilateral.ids.get(uid=uid,count=2000)
        except Exception ,e:
			print e
			current_index = int(random.random()*100 % self.length)
			self.client=self.access_client(current_index)
			return self.get_friend_bilateral_id(uid)    
